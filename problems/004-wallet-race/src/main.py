...
def transfer(req: TransferRequest, db: Session = Depends(get_db)):
    ...
    # Use SELECT ... FOR UPDATE to lock both rows and prevent race conditions
    source = db.query(Wallet).filter(Wallet.id == req.from_wallet_id).with_for_update().first()
    target = db.query(Wallet).filter(Wallet.id == req.to_wallet_id).with_for_update().first()
